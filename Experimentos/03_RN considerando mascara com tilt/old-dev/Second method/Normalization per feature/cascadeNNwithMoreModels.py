'''
    ...
    Description:
        Create a cascade using more models in different scenarios.
...
## Load the model
model1 = load_model("model_pout71.h5")
model2 = load_model("model_pout72.h5")
model3 = load_model("model_pout73.h5")
model4 = load_model("model_pout74.h5")
model5 = load_model("model_pout75.h5")
model6 = load_model("model_pout76.h5")
model7 = load_model("model_pout77.h5")
...
for i in range(2, int(len(columns_1)/2)+1):
	...
	y_out = models[0].predict(x_current)
	y_out = unnormalization(y_out[0], min_pout, max_pout, range_a, range_b)
    
	for e in range(1, len(models)):
		y_out_current = models[e].predict(x_current)
		y_out_current = unnormalization(y_out_current[0], min_pout, max_pout, range_a, range_b)
		for u in range(0, y_out.shape[1]):
			y_out[0][u] = y_out[0][u] + y_out_current[0][u]

	for e in range(0,  y_out.shape[1]):
		 y_out[0][e] = (y_out[0][e])/len(models)
...
plt.savefig('ScenariosNNWithMore7Models.png', dpi = 200)
