...
### Using all data for training set
array_x = []
array_y = []
...
for i in range(2, len(columns_1)-5):
	...
	### Running regression and capturing error for each amplifier
	y_out = np.zeros((p_in.shape[0], p_in.shape[1]))

	for j in range(0, p_in.shape[1]):
		y_out[0, j] = svr_model[j].predict(x_in)
...
plt.savefig('ScenariosSVRWithMiddle(modelPerChannel).png', dpi = 200)
plt.show()