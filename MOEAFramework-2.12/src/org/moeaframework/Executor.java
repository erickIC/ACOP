...
public class Executor extends ProblemBuilder {
     ...
     * algorithms executed by this executor; or {@code null} if no
     ...
     * solution evaluations. The caller is responsible for ensuring the executor
     * service is shutdown after use.
     * 
    ...
    /**
     * Sets the checkpoint file where the algorithm state is stored. This method
     * must be invoked in order to enable checkpoints.
     * 
     ...
     *            the checkpoint frequency, specifying the number of evaluations
     *            between checkpoints
     * @return a reference to this executor
     */
    ...
    /**
     * Runs this executor with its configured settings multiple times, returning
     * the individual end-of-run approximation sets. If the run is canceled, the
     * list contains any complete seeds that finished prior to cancellation.
     * 
     * @param numberOfSeeds
	...
	for (int i = 0; i < numberOfSeeds && !isCanceled.get(); i++) {
		...
		i--;
		System.out.println("Repeat seed " + i);
		continue;
	    }
     ...
     * @param numberOfSeeds
     *            to total number of seeds being run
	...
	try {
		...
		try {
			...
			algorithm = AlgorithmFactory.getInstance().getAlgorithm(algorithmName,
				properties.getProperties(), problem);
		    } else {
			algorithm = algorithmFactory.getAlgorithm(algorithmName, properties.getProperties(), problem);
		    }
