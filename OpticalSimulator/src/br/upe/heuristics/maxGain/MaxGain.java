package br.upe.heuristics.maxGain;
...
public class MaxGain extends ACOPHeuristic {	

    public MaxGain(int numberOfAmplifiers, float[] linkLosses, OpticalSignal inputSignal, ObjectiveFunction function) {
	...
	for (int i = 0; i < numberOfAmplifiers; i++) {
		...
		// a VOA for the link, not for the AMP.
		...
		if (amplifiers[i - 1] instanceof AmplifierVOA) {
			...
			// (decrease the gain)
			float newGain = amplifiers[j].getGain() - 0.5f;//Math.round(amplifiers[j].getGain() - difference);
	...
	float[] linLosses = { 19.00f, 23.00f, 20.00f, 14.00f, 15.00f, 15.00f, 23.00f, 16.00f, 16.00f, 20.00f, 15.00f,
	...
	PowerMaskSignal signal = new PowerMaskSignal(numberCh, AmplifierType.EDFA_1_PadTec, simSet.getCHANNEL_POWER(),
		40);
