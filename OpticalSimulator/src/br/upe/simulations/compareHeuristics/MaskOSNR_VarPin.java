...
public class MaskOSNR_VarPin {
	...
	while (pinSystem >= -21.0f) {
	    ...
	    double nfWeight = 1; double gfWeight = 0.5;

	    heuristic = new AdGC(numberAmplifiers, linLosses, inputSignal,
		    function);
	    heuristic.setInitialization(new UniformInitialization(type));
	    ...
	    heuristic.setRoadmAttenuation(simSet.getROADM_ATT());
	    heuristic.setMaxOutputPower(maxPout);
	    ((AsHBFlex) heuristic).setMaxIteration(10); // When the selection
							// uses weight
	    if (heuristic.getSelectionOp() instanceof OSNRWeightSelection) {
