...
public class SignalFeatureCalculation {
	...
	public static double calculateMinPower(OpticalSignal signal) {
		double minPower = Double.MAX_VALUE;
		for (OpticalChannel c : signal.getChannels()) {
			if (c.getSignalPower() < minPower) {
				minPower = c.getSignalPower();
			}
		}
	...
	public static double calculateMaxPower(OpticalSignal signal) {
		double maxPower = Double.MIN_VALUE;
		for (OpticalChannel c : signal.getChannels()) {
			if (c.getSignalPower() > maxPower) {
				maxPower = c.getSignalPower();
			}
		}
	...
	public static double calculateRippleOSNR(OpticalSignal signal) {
		return calculateMaxOSNR(signal)-calculateMinOSNR(signal);
	}
