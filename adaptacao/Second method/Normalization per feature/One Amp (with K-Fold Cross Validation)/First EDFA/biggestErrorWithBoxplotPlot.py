...
for i in range(0, len(pred_41)):
    ...
    xio_end.append(fold42_x[(k-1)-i])
    xio_end2.append(fold41_x[(k-1)-i])
    #print(foldico_x[(k-1)-i])
...
for i in range(0 , len(pred_yio)):
    ...
    while j < len(pred_yio[i]):
        ...
        if biggest_current > biggest_bigger:
            biggest_bigger = biggest_current
            biggest_test = current_t
            biggest_pred = current
            biggest_i = i 
            biggest_j = current_j
...
biggest_in = np.array([xio_end[biggest_i][biggest_j]])
biggest_in2 = np.array([xio_end2[biggest_i][biggest_j]])
print(biggest_bigger)
print(biggest_in, len(biggest_in))
...
plt.title('PerChannel EDFA 1 Biggest Error Spectrum')
plt.tight_layout()
plt.legend()
plt.grid(True)
plt.savefig('plots/PerchannelEDFA1Spectrum.pdf', dpi = 200)
'''

n42_pred_biggest = unnormalization(models42to40[0].predict(biggest_in), min_pout, max_pout, range_a, range_b)[0]
n41_pred_biggest = unnormalization(models41to40[0].predict(biggest_in2), min_pout, max_pout, range_a, range_b)[0]


print(n42_pred_biggest)

plt.figure(figsize=(10,8))

plt.plot(wavelength, biggest_pred,'o-' ,label='PerChannel', linewidth=4, markersize = 10)
plt.plot(wavelength, n41_pred_biggest,'o-' ,label='Spectrum', linewidth=4, markersize = 10)
plt.plot(wavelength, n42_pred_biggest,'o-' ,label='Spectrum-tilt', linewidth=4, markersize = 10)
plt.plot(wavelength, biggest_test,'o-' ,label='Expected', linewidth=4, markersize = 10)
...
plt.legend()
plt.grid(True)
plt.savefig('plots/EDFA1SpectrumWithPerChannel.png', dpi = 200)