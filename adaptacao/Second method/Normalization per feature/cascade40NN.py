...
for i in range(2, int(len(columns_1)/2)+1):
	...
	y_out = []
	for k in range(0, number_channels):
		y_current = models[k].predict(x_current)
		aux = unnormalization(y_current[0], min_pout, max_pout, range_a, range_b)
		y_out.append(aux[0])
	y_out = np.array(y_out)
	

	#print(y_out.shape[0], y_out.shape[1])
	...
	for j in range(0, len(y_out)):
		aux = normalization((y_out[j] - loss_1), min_pin, max_pin, range_a, range_b)
		signal_current.append(aux[0])
	gset_current.append(normalization(gain_1, min_gset, max_gset, range_a, range_b))
	gset_current = np.array([gset_current])
	signal_current = np.array([signal_current])
...
plt.plot(wavelength, y_out,'o-', label='predicted last')
plt.plot(wavelength, dataframe_1[columns_1[len(columns_1) - 1]], 'o-',label='expected last')
plt.ylabel('Pout (dBm)')
...
for i in range(2, len(columns_2)):
	...
	gset_current = []
	for j in range(0, len(y_out)):
		aux = normalization((y_out[j] - loss_2), min_pin, max_pin, range_a, range_b)
		signal_current.append(aux[0])
	gset_current.append(normalization(gain_2, min_gset, max_gset, range_a, range_b))
	gset_current = np.array([gset_current])
	signal_current = np.array([signal_current])
...
plt.plot(wavelength, y_out,'o-', label='predicted last')
plt.plot(wavelength, dataframe_2[columns_2[len(columns_2) - 1]], 'o-',label='expected last')
plt.ylabel('Pout (dBm)')
plt.xlabel('Wavelenght')
plt.grid(True)
plt.legend()
...
for i in range(2, len(columns_3)):
	...
	diff_current = []
	for j in range(0, y_out.shape[0]):
		diff_current.append(abs(float(y_out[j]) - dataframe_3[columns_3[i]][j]))
	...
	for j in range(0, len(y_out)):
		aux = normalization((y_out[j] - losses_3[i-1]), min_pin, max_pin, range_a, range_b)
		signal_current.append(aux[0])
	signal_current = np.array([signal_current])
...
plt.plot(wavelength, y_out,'o-', label='predicted last')
plt.plot(wavelength, dataframe_3[columns_3[len(columns_3) - 1]], 'o-',label='expected last')
plt.ylabel('Pout (dBm)')
plt.xlabel('Wavelenght')
plt.grid(True)
plt.legend()

plt.savefig('Scenarios40NNWithMiddle.png', dpi = 200)
